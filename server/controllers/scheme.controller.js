...
// Build MongoDB filter query based on eligibility parameters
...
export const getSchemes = async (req, res, next) => {
  try {
    ...
    // Check search Cache first (only if it's not semantic, which is custom processed)
    ...
    if (search && searchType === 'semantic') {
      ...
      // Fetch matching schemes, bringing back the hidden embedding vector
      const matchedFilterSchemes = await schemeRepository.find(filters, '', {}, null, null, '+embedding');

      // Compute local manual cosine similarity
